...
// basierend auf Sketch aus MAKE: 06/2023 S. 64 
...
#define PROJECT "OBEGRÄNSAD (ESP8266)"
...
bool  OE_ = LOW; // LOW = Anzeige EIN, HIGH = Anzeige AUS
...
void onConnectionEstablished()
{
  client.subscribe("obegraensad/setDisplay", [](const String & payload) {
    if (payload == "ON") OE_ = false; else OE_ = true;
    set_display();   
    p_scan();
    digitalWrite(P_EN, OE_);
    if (!OE_) analogWrite(P_EN, brightness_steps[brightness]);  // Gewählte bzw. vorherige Helligkeit einstellen
  });

  client.subscribe("obegraensad/display-state", [](const String & payload) {
    String state;

    if (OE_) state = "OFF"; else state = "ON";
...
void loop() {
  ...
  if (second == 0) {
    set_display();   
    if (!OE_) p_scan(); // refreshes display
    delay(500);
  }
  delay(100);
